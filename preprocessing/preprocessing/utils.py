...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                return df.groupby('tag_name').filter(lambda group: len(group) >= x)
            return filter_function
    ...
    def get_train_batch(self):
        ...
        next_x = self.preprocess_text(self.x_train[start:end])
        # TODO: CODE HERE
        # takes y_train between i * batch_size to (i + 1) * batch_size
        next_y = self.y_train[start:end]
    ...
    def get_test_batch(self):
        ...
        next_x = self.preprocess_text(self.x_test[start:end])
        next_y = self.y_test[start:end]
